...
initial_position SDiceWithDeath = do
  d1 <- d6
  d2 <- d6
  d3 <- d6
  d4 <- d6
  return $ DiceWithDeath d1 d2 d3 d4
...
do_move Accept b@(Board n s (Crypt d1 d2 d3))
    | strength s >= d1 + d2 + d3 = move_to SDiceWithDeath b
...
do_move Accept b@(Board n s p@(DiceWithDeath d1 d2 d3 d4))
    ...
    | d1 + d2 == d3 + d4 = move_to SDiceWithDeath b
    | otherwise = move_to SDiceWithDeath (Board n (lose_life 1 s) p)
